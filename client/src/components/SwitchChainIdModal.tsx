...
export default function SwitchChainIdModal() {
  const [isChainIdModalOpen, toggleChainIdModal, setChainIds] =
    useNetworkStore(
      useShallow((state) => [
        state.isChainIdModalOpen,
        state.switchNetwork,
        state.toggleChainIdModal,
        state.setChainIds,
      ])
    );

  const closeModal = () => {
    toggleChainIdModal(false);
  };

  const handleSetChainIds = (chainOption: number) => {
    if (chainOption === 1) {
      setChainIds(CHAIN_IDS.sepolia);
    } else if (chainOption === 2) {
      setChainIds(CHAIN_IDS.mumbai);
    }
  };
